unit clase_LogicaFactura;
//DaMoRaDEV, SDK VeriFactu 0.1.0
   {
     Logica Factura Basada en el documento de la AEAT:
     FAQs_ver_1_1_facturas.pdf
     Recordamos que es un SDK esta lógica es aproximada,
     en en diseño del software se debe atender a la logica
     de la factura que se genera, asi como los calculos pertinentes de la misma.
   }
interface
 ...
 Begin
          ...
          if FTipoFacturaRectificada='F2' then
              Begin
                 ...
                 if Destinatarios.NodoNecesario=True then
                     raise Exception.Create('Error LogicaFactura: Este tipo de factura F1 NO requiere Destinatario ');
              End
          else
              Begin
                   // la factura F1 requiere destinatario
                 if Destinatarios.NodoNecesario = false then
                      raise Exception.Create('Error LogicaFactura: Este tipo de factura F1 requiere Destinatario ');
              End;
       End;
   ...
   if RegistroAlta.TipoFactura ='F3'  then
     Begin
       ...
       if FTipoFacturaRectificada='F2' then
            if Destinatarios.NodoNecesario=true then
                  raise Exception.Create('Error LogicaFactura: La factura F3 no requiere Destinatarios ya que sustituye a F2');
     End;
